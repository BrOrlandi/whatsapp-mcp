...
| An installer-created account | Marked as needing a password change. The panel serves nothing but the password form until it is replaced, so a temporary password left in a terminal scrollback buys an attacker a form. |
